 ...
 * ListUtil.kt is part of Music Player.
 *
    ...
    when (this) {
        is Name.Known ->
            tokens.firstOrNull()?.let {
                if (it.value.all { char -> char.isDigit() }) "#" else it.value.first().uppercase()
            }
        is Name.Unknown -> "?"
    }
...
class CookieShapeDrawable(context: Context) : Drawable() {
    ...
    private val paint =
        Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = context.getAttrColorCompat(MR.attr.colorSurfaceVariant).defaultColor
